...
class DummyClassification:
    def __init__(self, category_name):
        self.label = category_name

class DummyHandedness:
    def __init__(self, category_name):
        self.classification = [DummyClassification(category_name)]
...
def main():
    ...
    options = vision.HandLandmarkerOptions(
        ...
        running_mode=vision.RunningMode.IMAGE if use_static_image_mode else vision.RunningMode.VIDEO,
        num_hands=2,
    ...
    while True:
        ...
        if use_static_image_mode:
            ...
            # Requires a timestamp for video mode
        ...
        if results.hand_landmarks:
            for hand_landmarks, handedness in zip(results.hand_landmarks,
                                                  results.handedness):
                # Wrap the results for compatibility with the rest of the code
                hand_landmarks = DummyLandmarks(hand_landmarks)
                handedness = DummyHandedness(handedness[0].category_name)
                ...
                # Track all 21 keypoints (42 coordinates) for dynamic movement analysis
                ...
                # Calculate maximum pixel movement across all 21 keypoints
                # This prevents stationary hands from triggering dynamic gestures (since you have no 'Stop' data)
                movement_distance = 0
                ...
                # If the hand has moved less than 50 pixels in the last 16 frames, it's stationary!
                if movement_distance < 50:
                    finger_gesture_id = 0
                elif len(valid_frames) < 10:
                    # Need at least 10 valid frames of data to make a decision
                    finger_gesture_id = 0
                else:
                    # Gate dynamic gestures with their corresponding static shapes AND geometric rules
                    if finger_gesture_id == 1:  # Hungry
                        if hand_sign_id != 5:  # Must be C-shape (5)
                            finger_gesture_id = 0
                        else:
                            # Hungry must be a clear downward movement (y increases in OpenCV)
                            wrist_y_start = valid_frames[0][1]
                            wrist_y_end = valid_frames[-1][1]
                            if wrist_y_end < wrist_y_start + 40:  # Must move down at least 40 pixels
                                finger_gesture_id = 0
                    elif finger_gesture_id == 2:  # SOS
                        # SOS must end with a closed fist (1)
                        if hand_sign_id != 1:
                            finger_gesture_id = 0
                        else:
                            # SOS fingers must fold inward: distance from index tip to wrist decreases
                            # Wrist: indices 0,1. Index tip: indices 16,17
                ...
                # Only display dynamic text if it's NOT the Stop class (0)
                dynamic_text = point_history_classifier_labels[most_common_fg_id[0][0]]
...
def draw_point_history(image, point_history):
    for index, point in enumerate(point_history):
        # Draw only the wrist trajectory to avoid lagging
        if len(point) >= 2 and point[0] != 0 and point[1] != 0:
            cv.circle(image, (int(point[0]), int(point[1])), 1 + int(index / 2),
                      (152, 251, 152), 2)
